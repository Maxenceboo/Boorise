...
const stats = [
  { label: "Devis en cours", value: "12", detail: "4 a relancer", icon: FileText },
  { label: "CA previsionnel", value: "18 420 €", detail: "+12% ce mois", icon: Euro },
  { label: "Materiaux suivis", value: "126", detail: "9 prix a verifier", icon: PackageCheck },
];

export function DashboardPage() {
  return (
    <div className="space-y-6">
      <div className="overflow-hidden rounded-lg border bg-violet-surface text-white shadow-sm">
        <div className="grid gap-6 p-6 lg:grid-cols-[1fr_320px] lg:p-8">
          <div>
            ...
            <h1 className="mt-4 max-w-2xl text-3xl font-semibold leading-tight">
              Les devis, clients et achats chantier au meme endroit.
            </h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-violet-100">
              Une interface pensee pour travailler vite : suivi des devis, relances et preparation
              des quantites sans perdre le contexte.
            </p>
          ...
          <div className="rounded-lg border border-white/10 bg-white/8 p-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-sm text-violet-100">A traiter aujourd'hui</div>
                <div className="mt-1 text-2xl font-semibold">7 actions</div>
              </div>
              <Clock3 className="h-8 w-8 text-accent" />
            </div>
            <div className="mt-5 space-y-3">
              {["Relancer devis D-2026-014", "Verifier prix bois terrasse", "Envoyer PDF client Martin"].map(
                (item) => (
        ...
        <Card>
          ...
          <CardContent>
            <div className="overflow-hidden rounded-md border">
              ...
              {[
                ["Martin Renovation", "Envoye", "4 820 €", "Relancer"],
                ["SCI Les Pins", "Brouillon", "9 340 €", "Completer"],
                ["Claire Dubois", "Accepte", "2 180 €", "Facturer"],
              ].map(([client, status, total, action]) => (
                <div
                  key={client}
                  className="grid grid-cols-[1fr_120px_120px_120px] items-center border-t px-4 py-3 text-sm"
                >
                  <span className="font-medium">{client}</span>
                  <Badge variant={status === "Accepte" ? "default" : "secondary"}>{status}</Badge>
                  <span>{total}</span>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              {["Organisation", "Clients", "Materiaux", "Calculs", "Devis", "PDF"].map((item, index) => (
